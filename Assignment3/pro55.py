...
train_sol = ( train_sol - np.min(train_sol) ) / (np.max(train_sol) - np.min(train_sol))
val_img = ( val_img - np.min(val_img) ) / (np.max(val_img) - np.min(val_img))
val_sol = ( val_sol - np.min(val_sol) ) / (np.max(val_sol) - np.min(val_sol))
test_img = ( test_img - np.min(test_img) ) / (np.max(test_img) - np.min(test_img)) 
test_sol = ( test_sol - np.min(test_sol) ) / (np.max(test_sol) - np.min(test_sol)) 
...
train_y = tf.cast(train_img, dtype=tf.int32)
val_y = tf.cast(val_img, dtype=tf.int32)
test_y = tf.cast(test_img, dtype=tf.int32)
